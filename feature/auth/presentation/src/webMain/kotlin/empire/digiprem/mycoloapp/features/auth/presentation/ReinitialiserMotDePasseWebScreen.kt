...
@Composable
fun ReinitialiserMotDePasseWebScreen(
    onNavigateBack: () -> Unit,
    ...
    LaunchedEffect(Unit) {
        viewModel.events.collect { event ->
            when (event) {
                ReinitialiserMotDePasseEvent.OnLienEnvoye -> {}
    ...
    WebFormPageScaffold(
        ...
        description = "Entrez votre email pour recevoir le code de réinitialisation",
    ) {
        FormScaffold(
            errorMessage = state.errorMessage,
            onCleanErrorClick = { viewModel.onAction(ReinitialiserMotDePasseAction.OnEffacerErreur) },
            footer = {
                if (state.emailEnvoye) {
                    MyColoButton(
                        ...
                        isLoading = false,
                        enabled = true,
                        onClick = { viewModel.onAction(ReinitialiserMotDePasseAction.OnNaviguerVersVerification) },
                    )
                } else {
                    MyColoButton(
                        ...
                        onClick = { viewModel.onAction(ReinitialiserMotDePasseAction.OnEnvoyerLienClick) },
                    )
                }
            }
        ) {
