...
import empire.digiprem.mycolowepapp.core.design_system.datalist.DataListScreen
...
private fun AdminDashboardState.toParticipantListState(): ListState<Participant> = ListState(
...
@Composable
private fun participantColumns(): List<ColumnDef<Participant>> = listOf(
    ColumnDef(
        key = "fullName",
        header = "Prénom",
        render = { p -> Text(p.fullName, style = MaterialTheme.typography.bodySmall, fontWeight = FontWeight.Medium) }
    ),
    ColumnDef(
        key = "familyName",
        header = "Nom",
    ...
    ColumnDef(
        ...
        render = { p ->
            val formatted = "%02d/%02d/%04d".format(p.birthDate.dayOfMonth, p.birthDate.monthNumber, p.birthDate.year)
