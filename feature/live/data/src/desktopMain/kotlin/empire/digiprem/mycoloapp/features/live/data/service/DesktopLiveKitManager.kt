...
class DesktopLiveKitManager : LiveKitManager {
    ...
    override suspend fun joinRoom(roomName: String, token: String, livekitUrl: String): Result<Unit, LiveError> {
        ...
        openBrowserLive(livekitUrl, token, isPublishing = false)
    ...
    override suspend fun publishCamera(): Result<Unit, LiveError> {
        webData = webData?.let {
            openBrowserLive(it.livekitUrl, it.token, isPublishing = true)
    ...
    override suspend fun toggleCamera(): Result<Unit, LiveError> = Result.Success(Unit)
    ...
    private fun openBrowserLive(livekitUrl: String, token: String, isPublishing: Boolean) {
        try {
            val encodedUrl = URLEncoder.encode(livekitUrl, "UTF-8")
