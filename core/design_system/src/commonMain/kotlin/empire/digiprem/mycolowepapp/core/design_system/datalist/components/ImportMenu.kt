...
fun ImportMenu(
    ...
    Box(modifier = modifier) {
        ...
        DropdownMenu(expanded = expanded, onDismissRequest = { expanded = false }) {
            DropdownMenuItem(text = { Text("Importer CSV") }, onClick = { expanded = false; onImport(ImportFormat.CSV) })
            DropdownMenuItem(text = { Text("Importer XLSX") }, onClick = { expanded = false; onImport(ImportFormat.XLSX) })
        }
    }
}
