...
suspend fun onNavHostReady(navController: NavController) {
    val hash = window.location.hash.removePrefix("#").removePrefix("/").substringBefore("?")

    when {
        hash.isEmpty() || hash == "landing" -> {
            //Landing est le startDestination — rien à faire
        }
        ...
        hash.startsWith("confirmation")    -> {
            //Nécessite un referenceNumber — redirige vers Landing
        }
